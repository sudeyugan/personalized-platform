...
class UnconfiguredProvider implements AiProvider {
  ...
  async testConnection() {
    ...
    return `配置已就绪：${this.model || '未命名模型'}。实际请求将在选定兼容服务协议后开放。`
  }
...
export function createAiProvider(config: { providerId: 'mock' | 'custom'; endpoint: string; model: string }): AiProvider { return config.providerId === 'mock' ? new MockProvider() : new UnconfiguredProvider(config.endpoint, config.model) }
