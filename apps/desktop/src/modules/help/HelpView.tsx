...
const sections: HelpSection[] = [
  ...
  },
  {
    ...
    guides: [
      { title: '开始对话与新对话', body: '在主界面伙伴区输入文字，或单击桌面形象打开轻量聊天。聊天右上角可以新开对话；生成过程中可以中止当前回答，语音朗读可以暂停、继续或停止。DeepSeek 回复会逐段显示；需要调用工具时，查询前的临时文字会被最终整合回答替换，语音只朗读最终回答一次。' },
      ...
      { title: '电脑能力与权限', body: '设置 → AI 伙伴 → 权限与记录 → 电脑能力中可开启。窗口、鼠标键盘、剪贴板、文件、程序、PowerShell、截图与录屏都通过统一 Tool 和双层权限检查；文件与程序可以按目录或目标长期授权，删除、关闭、移动、停止进程和 PowerShell 始终逐次确认。主开关默认关闭。明确说“打开 bilibili.com”时会直接进入受控打开流程，普通域名会在本地补为 HTTPS；“已打开”仅表示 Windows 默认程序已接收请求。' },
      { title: '录屏、提醒与紧急停止', body: '截图和录屏使用本机 FFmpeg，可自动探测 D 盘安装；默认输出到视频目录的一隅录制文件夹。开启后台事务提醒后，主窗口关闭到托盘仍会按日历时间或待办日期提醒。托盘“立即停止小鱼操作”或设置中的紧急快捷键会中止由小鱼启动的录屏和后台进程；应用真正退出后不再提醒。' },
      ...
      { title: '静态立绘与动态 WebM', body: '可以上传单张透明 PNG/WebP，或使用 9:16、VP9、静音且带透明通道的 WebM；960×1708 和 1792×3184 都可使用。动作库支持待机、倾听、回应、庆祝、关切、招呼、环顾、点头、害羞、困倦、伸懒腰和打哈欠。待机是桌面主画面，环顾和伸展会隔一段时间自然穿插，夜间才低频出现哈欠或困倦；模型短暂处理时保持待机，不会强制切换。缺少任何状态时回退待机。单击聊天、拖动移动、双击返回主窗口；默认 Ctrl+Alt+Y 显示或隐藏，Ctrl+Alt+T 可在安静穿透与原显示方式之间切换，两者都可在设置中更换或关闭。' },
      { title: '语音、唤醒词与声纹', body: '配置 ElevenLabs 的语音识别、合成 Key 与 Voice ID 后，可以录音转写并流式朗读回答。语音唤醒默认关闭；首次启用需下载约 43 MB 本地模型，并可录入四段声音。待机时唤醒词与声纹都只在本机判断，通过后才连接 ElevenLabs。伙伴回答或朗读时可以开口打断，但会先确认约半秒的有效话语；填充音、短杂音和扬声器回声不会作为问题提交。唤醒后的交谈可选单轮、默认约 8 秒追问的短连续或约 15 秒追问的持续模式。开启唤醒后关闭主窗口只收进托盘，仍可说唤醒词；托盘“退出一隅”才会停止监听。应用不会开机自启动，重启后需手动启动一次。' },
