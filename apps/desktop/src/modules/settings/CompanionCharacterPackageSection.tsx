...
export function CompanionCharacterPackageSection() {
  ...
  const importPackage = async (files: FileList | null) => {
    ...
    try {
      ...
      await validateCharacterCanvas(plan)
      ...
      setStatus(`已导入 ${resolved.name}：${assetIdByPath.size} 张素材，${Object.keys(resolved.motions).length} 个动作。`)
  ...
  return <div className="character-package-settings">
    ...
    {character && <div className="character-package-preview">
      <CharacterRuntime ref={runtime} character={character} urls={urls} label={`${character.name} 角色包预览`} />
      <div><strong>{character.name}</strong><small>{character.canvas.width} × {character.canvas.height} · 差分立绘 {Object.keys(character.expressions).length} 种表情 · 固定动作 {Object.keys(character.motions).length} 个</small>
