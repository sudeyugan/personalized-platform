...
export function ComputerCapabilitySettings() {
  ...
  return <details className="permission-scope-group computer-capability-settings">
    <summary><span>电脑能力</span><small>{settings.enabled ? '已开启' : '未开启'}</small></summary>
    <div className="computer-capability-body">
      <div className="computer-master-row">
        <MonitorCog />
        <span><strong>允许小鱼操作这台电脑</strong><small>窗口、输入、文件、进程与录屏统一经过 Tool Registry 和权限检查</small></span>
      ...
      {settings.enabled && <>
        <label className="computer-field"><span><strong>权限模式</strong><small>受信任工作站只自动执行可见、可恢复的桌面操作</small></span><select value={settings.profile} onChange={(event) => setCompanionComputer({ profile: event.target.value as typeof settings.profile })}><option value="trusted_workstation">受信任工作站</option><option value="standard">标准</option><option value="custom">仅按规则</option></select></label>
        <label className="computer-field"><span><strong>FFmpeg</strong><small>用于截图与录屏；已支持自动探测 D 盘</small></span><span className="computer-inline"><input value={settings.ffmpegPath} placeholder="D:\ffmpeg\bin\ffmpeg.exe" onChange={(event) => setCompanionComputer({ ffmpegPath: event.target.value })} /><button onClick={() => void detectFfmpeg()}>自动查找</button></span></label>
        <label className="computer-field"><span><strong>录制目录</strong><small>留空时保存到“视频\一隅录制”</small></span><input value={settings.recordingDirectory} placeholder="留空使用默认目录" onChange={(event) => setCompanionComputer({ recordingDirectory: event.target.value })} /></label>
        <div className="computer-master-row"><Video /><span><strong>后台事务提醒</strong><small>程序隐藏到托盘后仍会提醒；提前 {settings.reminderLeadMinutes} 分钟</small></span><button aria-label="后台事务提醒" aria-pressed={settings.backgroundReminders} className={settings.backgroundReminders ? 'switch on' : 'switch'} onClick={() => setCompanionComputer({ backgroundReminders: !settings.backgroundReminders })}><i /></button></div>
        {settings.backgroundReminders && <label className="computer-field compact"><span>提前分钟数</span><input type="number" min={0} max={1440} value={settings.reminderLeadMinutes} onChange={(event) => setCompanionComputer({ reminderLeadMinutes: Math.max(0, Math.min(1440, Number(event.target.value) || 0)) })} /></label>}
        <label className="computer-field compact"><span><strong>紧急停止快捷键</strong><small>中止小鱼启动的录屏和后台程序</small></span><select value={settings.emergencyShortcut} onChange={(event) => setCompanionComputer({ emergencyShortcut: event.target.value })}><option value="CommandOrControl+Alt+Escape">Ctrl + Alt + Esc</option><option value="CommandOrControl+Shift+Escape">Ctrl + Shift + Esc</option><option value="CommandOrControl+Alt+S">Ctrl + Alt + S</option></select></label>

        <details className="computer-grant-editor">
          <summary><FolderKey />目录与程序授权</summary>
          <div>
